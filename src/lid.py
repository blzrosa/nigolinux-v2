...
def disable_lid_switch():
    ...
    for i, line in enumerate(lines):
        ...
        if stripped_line.startswith(
            "HandleLidSwitch="
        ) or stripped_line.startswith("#HandleLidSwitch="):
            new_lines.append("HandleLidSwitch=ignore\n")
            found[0] = True

        elif stripped_line.startswith(
            "HandleLidSwitchExternalPower="
        ) or stripped_line.startswith("#HandleLidSwitchExternalPower="):
            new_lines.append("HandleLidSwitchExternalPower=ignore\n")
            found[1] = True

        elif stripped_line.startswith(
            "HandleLidSwitchDocked="
        ) or stripped_line.startswith("#HandleLidSwitchDocked="):
    ...
    if login_section_index == -1:
        ...
        if not found[0]:
            new_lines.insert(
                login_section_index + 1, "HandleLidSwitch=ignore\n"
            )
        if not found[1]:
            new_lines.insert(
                login_section_index + 1,
                "HandleLidSwitchExternalPower=ignore\n",
            )
        if not found[2]:
            new_lines.insert(
                login_section_index + 1, "HandleLidSwitchDocked=ignore\n"
            )
