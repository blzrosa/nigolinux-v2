...
def apply_sddm_theme():
    ...
    install({'pacman': ["sddm", "qt6-svg", "qt6-virtualkeyboard", "qt6-multimedia-ffmpeg"]})
    
    if THEME_DEST_DIR.exists():
        shutil.move(THEME_DEST_DIR, f"{THEME_DEST_DIR}_{subprocess.check_output(['date', '+%s']).strip().decode()}")
