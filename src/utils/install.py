...
def install(to_install: Dict[Installer, List[str]]) -> None:
    for installer, packages in to_install.items():
        ...
        match installer:
            case "pacman":
                for package in TypedProgressBar(
                    packages, packages, "Pacman install"
                ):
                    execute_as_root(
            ...
            case "flatpak":
                install_flatpak()
                for package in TypedProgressBar(
                    packages, packages, "Flatpak install"
                ):
                    execute_as_user(
...
def install_yay() -> None:
    ...
    execute_as_user(
        ["git", "clone", "https://aur.archlinux.org/yay.git", str(temp_dir)]
    )
