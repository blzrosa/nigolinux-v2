...
from src.cli.cli_utils import clear_console, get_key, print_menu, handle_quit
...
def run_tasks(task_id_set: Set[str]) -> None:
    ...
    if str(len(MASTER_TASK_ORDER)) in final_task_ids:
         tasks_to_run_map[str(len(MASTER_TASK_ORDER))] = TASKS[str(len(MASTER_TASK_ORDER))]
    ...
    for _, (description, task_function) in enumerate(tasks_to_run):
        try:
            ...
            if len(tasks_to_run) > 1:
                while True:
                    choice = input("Do you want to continue with the next tasks? (y/n): ").lower().strip()
                    if choice == 'n':
                        print("Aborting remaining tasks. Returning to main menu.")
                        return
                    elif choice == 'y':
...
def main() -> None:
    try:
        ...
        print(f"Error: {e}\nPlease run this script as root or with sudo.", file=sys.stderr)
        sys.exit(1)
