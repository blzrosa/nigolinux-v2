...
def apply_grub_theme() -> None:
    ...
    shutil.copytree(
        GRUB_ASSETS_DIR / "common", dest_theme_path, dirs_exist_ok=True
    )
    ...
    shutil.copytree(
        ...
        / f"assets-{ICON_NAME}"
        / f"icons-{asset_res}",
        dest_theme_path / "icons",
    )
    shutil.copytree(
        GRUB_ASSETS_DIR / "assets" / "assets-select" / f"select-{asset_res}",
        dest_theme_path,
        dirs_exist_ok=True,
    )
    shutil.copy(
        GRUB_ASSETS_DIR / "assets" / f"info-{asset_res}.png",
        dest_theme_path / "info.png",
    )
    ...
    for line in lines:
        ...
        for key, value in configs_to_set.items():
            if stripped_line.startswith(key + "=") or stripped_line.startswith(
                "#" + key + "="
            ):
